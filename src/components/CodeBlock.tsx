/**
 * CodeBlock - Syntax-highlighted code block component with copy button
 * Uses Prism.js for synchronous highlighting (works in both SSR and client)
 */
...
/**
 * Syntax-highlighted code block component with language label and copy button
 */
export function CodeBlock({ language = '', content, children }: CodeBlockProps) {
  ...
  // If no language, render plain code block without header
  if (!hasLanguage) {
    return (
      <pre>
  ...
  const normalizedLang = lang === 'html' ? 'markup' : 
                         lang === 'js' ? 'javascript' :
                         lang === 'ts' ? 'typescript' :
                         lang === 'sh' ? 'bash' : lang;

  // Get display name for language
  const displayLang = normalizedLang === 'markup' ? 'HTML' :
